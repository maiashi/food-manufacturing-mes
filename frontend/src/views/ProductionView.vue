<template>
  <div class="production">
    <div class="page-header">
      <h2>製造実行</h2>
      <el-button type="primary" @click="showCreateOrder">新発注</el-button>
    </div>
    ...
    <el-card>
      ...
      <el-table :data="orders" stripe>
        ...
        <el-table-column label="操作" width="280">
          <template #default="{ row }">
            <el-button v-if="row.status === 'PLANNED'" size="small" @click="startBatch(row)">開始</el-button>
            <el-button v-if="row.status === 'IN_PROGRESS'" size="small" type="success" @click="completeBatch(row)">完了</el-button>
            <el-button v-if="row.status === 'PLANNED' || row.status === 'IN_PROGRESS'" size="small" type="warning" @click="pauseBatch(row)">一時停止</el-button>
    ...
    <el-card v-if="selectedBatch" style="margin-top: 16px">
      ...
      <el-descriptions :column="2" border>
        <el-descriptions-item label="工程">{{ selectedBatch.steps?.map(s => s.name).join(', ') }}</el-descriptions-item>
        <el-descriptions-item label="投入量">{{ selectedBatch.inputQty }} {{ selectedBatch.unit }}</el-descriptions-item>
        <el-descriptions-item label="出力量">{{ selectedBatch.outputQty ?? '---' }} {{ selectedBatch.unit }}</el-descriptions-item>
        <el-descriptions-item label="歩留率">{{ selectedBatch.yieldRate ?? '---' }}%</el-descriptions-item>
      </el-descriptions>
    ...
    <el-dialog v-model="showCreateOrder" title="新製造指示書" width="700px">
      <el-form :model="newOrderForm" label-width="120px">
        <el-form-item label="ライン"><el-select v-model="newOrderForm.lineId" placeholder="選択"><el-option v-for="l in lines" :key="l.lineId" :label="l.lineName" :value="l.lineId" /></el-select></el-form-item>
        <el-form-item label="製品"><el-select v-model="newOrderForm.productId" filterable><el-option v-for="p in products" :key="p.productId" :label="p.productName" :value="p.productId" /></el-select></el-form-item>
        <el-form-item label="数量"><el-input-number v-model="newOrderForm.plannedQty" /></el-form-item>
        <el-form-item label="計画開始"><el-date-picker v-model="newOrderForm.plannedStart" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss" /></el-form-item>
      </el-form>
      <template #footer>
...
<style scoped>
.production { padding-bottom: 20px; }
