<template>
  <div class="production">
    ...
    >
      <template #header>
        バッチ詳細 - {{ selectedBatch.batchNumber }}
      </template>
      <el-descriptions
        :column="2"
        border
      >
        <el-descriptions-item label="工程">
          {{ selectedBatch.steps?.map(s => s.name).join(', ') }}
    ...
    >
      ...
      >
        ...
        <el-form-item label="製品">
          <el-select
            v-model="newOrderForm.productId"
            filterable
          >
            <el-option
              v-for="p in products"
      ...
      <template #footer>
        <el-button @click="showCreateOrder = false">
          キャンセル
        </el-button>
        <el-button
          type="primary"
